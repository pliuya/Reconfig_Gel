...
// gel paramerers
#define c0 0.000576  // crosslink density in the undeformed state
...
//Ramp temperature: 1. Slow ramp: every #step increase 1 degree from 20-36
//                  2. Fast ramp to 34
//                  3. Fast ramp to 36
bool   heating = true;//true: finalTemp > 20; false: finalTemp = 20

bool   fastRamp = true;
double rampSteps = (fastRamp==true)?15.:50.;
double currenttemp = 0;
double finalTemp = 36.;

//bump parameters: 40*10*16
double Lx = 101, Ly = 11, Lz = 32;
double bump_top_thick = 2.;
double shift = 0;
double Origin_Lx = Lx + shift*2.;
double bump_height = Lz - bump_top_thick;
double bump_radius = ((0.5*Origin_Lx)*(0.5*Origin_Lx) + bump_height*bump_height)/(2*bump_height);
double bump_center_x = 0.5*Lx;
double bump_center_y = bump_height-bump_radius;
double deltaX = 1.0;
double PHIIni = 0.566;////this value is useless since PHIIni calculated directly from Tfinal
//double PHIIni = 0.124282; //32-20
double phi0 = 0.1286;//0.025;//0.0503;
...
public:
	...
	double vSPTOT;
	double P0;
    int    e_type;//e_type = 0 bump, =1: otherwise
	Element();
	void update_volume();
...
void   rampParameter(double );// modify parameters while ramping temperature
//void   Read_XYZ();
void   Update_XYZ();
...
vector<struct_coord> origin_coord;
...
int main(int argc, char *argv[]) {
	...
	while (simulation_time <= t_end) {
		...
		count_dump++;
        //current temperature, then update parameter
        currenttemp = 20 + (int)(simulation_time/rampSteps);
        if (currenttemp > finalTemp) {
            currenttemp = finalTemp;
        }
        rampParameter(currenttemp);
...
*/
    //redefine temperature increase me
	c06 = c0 * deltaX / 12.0;
	...
	*/
	...
	for (i = 0; i < N_nodes; i++){
//read nodes coordinate and type
		vtkfile >> nodes[i]->position[0]>> nodes[i]->position[1]
		>>nodes[i]->position[2]>>nodes[i]->Pos_Node;
        origin_coord.push_back({nodes[i]->position[0],nodes[i]->position[1],nodes[i]->position[2],nodes[i]->Pos_Node});
	}
	vtkfile.getline(buffer, 100);
	vtkfile >> buffer;
	vtkfile >> N_elements;
...
//  Ya's change: no fiber
	...
	ifstream latevtkfile("0.gel.vtk");
	...
	latevtkfile >> nodes[i]->position[0]>> nodes[i]->position[1]
		...
		////////////////////////////////////////////////////////////////////////////////
    // Read late volume fraction and update phi
    bool isCellData = false;
    string line;
    element_index = 0;
    while(getline(latevtkfile,line)){
        ...
        if(isCellData == true && element_index<N_elements){
            double tempvalue;
            int temptype;
            tempdata>>tempvalue>>temptype>>ws;
            elements[element_index]->update_volume();
            elements[element_index]->phi = tempvalue;
            elements[element_index]->e_type = tempvalue;
            if (SPUNIFORM == 1)
                elements[element_index]->vSPTOT = SPTOT;
            element_index++;
        }
...
void CalculateVolumes() {
	for (long int element_index = 0; element_index < N_elements; element_index++) {
        if (elements[element_index]->e_type == 0) {
            elements[element_index]->update_volume();
            elements[element_index]->phi = Vol0_phi0 / elements[element_index]->volume;
        }
    }
...
void CalculateMobilities() {
	...
	for (long int node_index = 0; node_index < N_nodes; node_index++) {
		...
		if (MobilityonTemperatureON == 1)
            nodes[node_index]->mobility = nodes[node_index]->mobility * reducedtemp;
...
void CalculateVelocities() {
	...
	for (node_index = 0; node_index < N_nodes; node_index++) {
        ...
        else if(nodes[node_index]->Pos_Node==5){
            nodes[node_index]->velocity[0] = 0;
            nodes[node_index]->velocity[1] = 0;
        ...
        else if(nodes[node_index]->Pos_Node==4){
            nodes[node_index]->velocity[0] = 0;
...
void dump_configuration_vtk(int t_i) {
    ...
    stringstream strainname, stressname;

	//filename << setw(10) << setfill('0') << simulation_step << ".gel.vtk";
	filename << setw(10) << setfill('0') << t_i << ".gel.vtk";
    strainname << setw(10) << setfill('0') << t_i << ".strain.data";
    stressname << setw(10) << setfill('0') << t_i << ".stress.data";
...
void   rampParameter(double currenttemp){
    chi0 = (30.614891304347623 + 3.4181159420289857 * currenttemp) / (273.15 + currenttemp);
    reducedtemp = (currenttemp + 273.) / (22. + 273.);
}
